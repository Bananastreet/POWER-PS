...
public class Connection {
	...
	public static ArrayList<String> MacBanneds = new ArrayList<String>();
	...
	**/
	...
	public static void unMacBannedUser(String name) {
		String mac = getMac(name);
		if (mac != "") {
			MacBanneds.remove(mac);
			deleteFromFile("./Data/bans/MacBanneds.txt", mac);
		}
	}
	...
	**/
	...
	public static void banMac() {
		try {
			BufferedReader in = new BufferedReader(new FileReader("./Data/bans/MacBanneds.txt"));
			String data = null;
			try {
			while ((data = in.readLine()) != null) {
	...
	**/
	...
	public static void addMacToFile(String Name) {
		try {
			BufferedWriter out = new BufferedWriter(new FileWriter("./Data/bans/MacBanneds.txt", true));
		    try {
				out.newLine();
				out.write(Name);
		    } finally {
	...
	**/
	...
	public static String getMac(String name) {
		...
		try {
			...
			while ((text = reader.readLine()) != null && done == 0) {
				...
				if (line >= 6) {
					...
					if (text.toLowerCase().startsWith("macaddress")) {
						// MacForUser = token2;
						done = 1;
		...
		/*
		 * if(!error) { bannedMacs.remove(token2);
		 * deleteFromFile("./Data/bans/UUIDBans.txt", uidForUser);
		 * c.sendMessage(
		 * "@red@Un-UUID banned user "+name+" with the UUID address of "
		 * +uidForUser+"."); }
		 */
	}
	/**
