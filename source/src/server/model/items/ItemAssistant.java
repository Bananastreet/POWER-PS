...
public class ItemAssistant {
	...
	public void handleSpecialPickup(int itemId) {
		// c.sendMessage("My " + getItemName(itemId) + " has been recovered. I should
		// talk to the void knights to get it back.");
			...
			if ((((freeSlots() >= 1) || playerHasItem(item, 1)) && Item.itemStackable[item])
				...
				for (int i = 0; i < c.playerItems.length; i++) {
					if ((c.playerItems[i] == (item + 1)) && Item.itemStackable[item] && (c.playerItems[i] > 0)) {
						 ...
						 * c.getOutStream().writeWord(c.playerItems[i]); if (c.playerItemsN[i] > 254) {
						 * c.getOutStream().writeByte(255);
	...
	public void sendWeapon(int Weapon, String WeaponName) {
		...
		if (WeaponName.equals("Unarmed")) {
			...
			c.getPA().sendFrame126(WeaponName, 428);
				...
				|| c.playerEquipment[c.playerWeapon] == 15241 || c.playerEquipment[c.playerWeapon] == 18357
				|| c.playerEquipment[c.playerWeapon] == 13880 || c.playerEquipment[c.playerWeapon] == 13881
				|| c.playerEquipment[c.playerWeapon] == 13882 || c.playerEquipment[c.playerWeapon] == 13883
			...
			// c.getPA().sendFrame126(WeaponName, 1767);
				...
				|| c.playerEquipment[c.playerWeapon] == 20472 || c.playerEquipment[c.playerWeapon] == 11308
				|| WeaponName2.contains("Staff of light") || c.playerEquipment[c.playerWeapon] == 18349
				|| c.playerEquipment[c.playerWeapon] == 10887 || WeaponName2.contains("sword")) {
	...
	public void getRequirements(String itemName, int itemId) {
		...
		case 14484: // dclaw20120
			c.attackLevelReq = 60;
			return;

		case 20120: // dclaw20120
			c.attackLevelReq = 60;
			return;
	...
	public boolean is2handed(String itemName, int itemId) {
		...
		if (itemName.contains("godsword") || itemName.contains("claw") || itemName.contains("aradomin sword")
	...
	public boolean wearItem(int wearID, int slot) {
		...
		synchronized (c) {
			...
			if (wearID > 15085 && wearID < 15102) {
				 ...
				 * c.sendMessage("You must be in a clan chat channel to do that."); } return
				 * false;
				 */
	...
	public boolean bankItem(int itemID, int fromSlot, int amount) {
		...
		// if(c.storing && playerHasItem(itemID, amount, fromSlot) && c.occupied[1] ==
		// false)
		// {
		...
		/*
		 * if(c.tradeStatus == 1 && !c.inTrade) { c.getTradeAndDuel().(); c.
		 * sendMessage("AntiDupe: You tried to bank an item while dueling, duel closed."
		 * ); return false; }
		 */
		if (c.inTrade) {
			c.sendMessage("you cant bank in trade");
			return false;
		}
		if (c.inTrade) {
		...
		if (c.isShopping) {
			 ...
			 * if (c.lastAttackedBy != null) { c.
			 * sendMessage("AntiDupe: Cannot bank item, errorcode: You can't drop items in combat."
			 * ); return false; }
			 */
