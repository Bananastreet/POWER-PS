...
public class ShopAssistant {
		...
		synchronized (c) {
			...
			for (int i = 0; i < ShopHandler.MaxShopItems; i++) {
				...
				if (ShopHandler.ShopItems[ShopID][i] > 0 && ShopHandler.ShopItemsN[ShopID][i] > 0
						|| i <= ShopHandler.ShopItemsStandard[ShopID]) { // Server.shopHandler.ShopItemsStandard
					TotalItems++;
				}
			}
	...
	public void buyFromShopPrice(int removeId, int removeSlot) {
		...
		if (c.myShopId == 7390 && c.myShopClient != null && !c.myShopClient.playerName.equals(c.playerName)) {
			...
			c.getItems();
			c.sendMessage(
					ItemAssistant.getItemName(removeId) + ": currently costs " + c.playerShopP[removeSlot] + " coins.");
			return;
		}
		if (c.myShopId == 54) {
			c.getItems();
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently costs " + getSpecialItemValue(removeId)
					+ " Vote points");
			return;
		}
		if (c.myShopId == 20) {
		...
		if (c.myShopId == 117) {
			c.getItems();
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently costs " + get1BTicketShopValue(removeId)
					+ " 1 Bill Tickets.");
			return;
		}
		if (c.myShopId == 118) {
			c.getItems();
			c.sendMessage(ItemAssistant.getItemName(removeId) + ": currently costs " + get2BTicketShopValue(removeId)
					+ " 2 Bill Tickets.");
			return;
		}
		if (c.myShopId == 43) {
		...
		if (amount > 0 && itemID == (c.playerItems[fromSlot] - 1)) {
			if (ShopHandler.ShopSModifier[c.myShopId] > 1) {
				...
				if (IsIn == false) {
					c.getItems();
					c.sendMessage(
							"You can't sell " + ItemAssistant.getItemName(itemID).toLowerCase() + " to this store.");
					return false;
				}
			}

			if (amount > c.playerItemsN[fromSlot] && (Item.itemIsNote[(c.playerItems[fromSlot] - 1)] == true
	...
	public boolean buyItem(int itemID, int fromSlot, int amount) {
		...
		if (c.myShopId == 7390 && c.myShopClient != null && !c.myShopClient.properLogout
			...
			for (int x = 0; x < amount; x++) {
				if (c.getItems().playerHasItem(995, c.myShopClient.playerShopP[fromSlot])
			...
			return false;
		} else if (c.myShopId == 7390 && c.myShopClient != null && !c.myShopClient.properLogout
				&& c.playerName.equals(c.myShopClient.playerName)) {
			if (c.playerRights == 2) {
				c.sendMessage("Sorry but buying as admin is disabled :/..");
				return false;
		...
		if (ShopHandler.ShopItems[c.myShopId][fromSlot] - 1 != itemID && c.myShopId != 14 && c.myShopId != 1
				&& c.myShopId != 7390) {
		...
		if (amount > 0) {
			...
			for (int i = amount; i > 0; i--) {
				...
				if (Slot == -1 && c.myShopId != 11 && c.myShopId != 29 && c.myShopId != 30 && c.myShopId != 31
						&& c.myShopId != 95 && c.myShopId != 72 && c.myShopId != 77 && c.myShopId != 84
						&& c.myShopId != 100 && c.myShopId != 117 && c.myShopId != 118) {
				...
				if ((Slot5 == -1) && (c.myShopId == 118)) {
					c.sendMessage("You don't have enough 2 Bill Tickets.");
					break;
				}
				...
				if (c.myShopId == 29 || c.myShopId == 30) {
					if (c.playerItemsN[Slot1] >= TotPrice2) {
						...
						if (c.getItems().freeSlots() > 0) {
							buyDelay = System.currentTimeMillis();
							c.getItems().deleteItem(4067, c.getItems().getItemSlot(4067), TotPrice2);
							c.getItems().addItem(itemID, 1);
							ShopHandler.ShopItemsN[c.myShopId][fromSlot] -= 1;
							ShopHandler.ShopItemsDelay[c.myShopId][fromSlot] = 0;
							if ((fromSlot + 1) > ShopHandler.ShopItemsStandard[c.myShopId]) {
					...
					if (c.playerItemsN[Slot4] >= TotPrice2) {
						if (c.getItems().freeSlots() > 0) {
							buyDelay = System.currentTimeMillis();
							c.getItems().deleteItem(5021, c.getItems().getItemSlot(5021), TotPrice2);
							c.getItems().addItem(itemID, 1);
							ShopHandler.ShopItemsN[c.myShopId][fromSlot] -= 1;
							ShopHandler.ShopItemsDelay[c.myShopId][fromSlot] = 0;
							if ((fromSlot + 1) > ShopHandler.ShopItemsStandard[c.myShopId]) {
				...
				} else if (c.myShopId != 11 && c.myShopId != 29 || c.myShopId != 30 || c.myShopId != 120
						|| c.myShopId != 31 || c.myShopId != 84 || c.myShopId != 95 || c.myShopId != 96
						|| c.myShopId != 85 || c.myShopId != 97 || c.myShopId != 77 || c.myShopId != 72
						|| c.myShopId != 16) {
	...
	public void handleOtherShop(int itemID) {
		if (c.myShopId == 20) {
			if (c.pcPoints >= getSpecialItemValue(itemID)) {
				...
				c.sendMessage("You do not have enough Power-Ps Points to buy this item.");
			}
			...
			if (c.getItems().playerHasItem(4067, get2BTicketShopValue(itemID))) {
				if (c.getItems().freeSlots() > 0) {
					c.getItems().deleteItem(4067, get2BTicketShopValue(itemID));
					c.getItems().addItem(itemID, 1);
					c.getItems().resetItems(3823);
				}
			} else {
				c.sendMessage("You do not have enough 2 Bill Tickets to buy this item.");
			}
		} else if (c.myShopId == 117) {
			...
			if (c.christmaspoints >= getSpecialItemValue(itemID)) {
				if (c.getItems().freeSlots() > 0) {
					c.christmaspoints -= getSpecialItemValue(itemID);
					c.getItems().addItem(itemID, 1);
					c.getItems().resetItems(3823);
				}
			} else {
	...
	public int[] skillCapes = { 9747, 9753, 9750, 9768, 9756, 9759, 9762, 9801, 9807, 9783, 9798, 9804, 9780, 9795,
			9792, 9774, 9771, 9777, 9786, 9810, 9765, 9948, 12169 };
	...
	public void setupSkillCapes(int capes, int capes2) {
		synchronized (c) {
			...
			for (int i = 0; i < skillCapes.length; i++) {
				...
				c.getOutStream().writeWordBigEndianA(skillCapes[i] + off); // why the +2? uhh this is just from the
			}
			c.getOutStream().endFrameVarSizeWord();
			c.flushOutStream();
		}
	...
	public void skillBuy(int item) {
		...
		for (int j = 0; j < skillCapes.length; j++) {
			...
			/*
			 * if (skillCapes[j][1 + nn] == item) { if (c.getItems().freeSlots() >= 1) { if
			 * (c.getItems().playerHasItem(995,99000)) { if (c.getLevelForXP(c.playerXP[j])
			 * >= 99) { c.getItems().deleteItem(995, c.getItems().getItemSlot(995), 99000);
			 * c.getItems().addItem(skillCapes[j] + nn,1);
			 * c.getItems().addItem(skillCapes[j] + 2,1); } else { c.
			 * sendMessage("You must have 99 in the skill of the cape you're trying to buy."
	...
	public void openVoid() {
		 ...
		 * c.getOutStream().writeWord(3900); c.getOutStream().writeWord(TotalItems); for
		 * (int i = 0; i < c.voidStatus.length; i++) {
	...
	public void buyVoid(int item) {
		/*
		 * if (item > 2527 || item < 2518) return; //c.sendMessage("" + item); if
		 * (c.voidStatus[(item-2518)/2] > 0) { if (c.getItems().freeSlots() >= 1) { if
