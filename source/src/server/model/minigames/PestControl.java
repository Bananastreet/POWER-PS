...
public class PestControl {
		...
		for (int j = 0; j < PlayerHandler.players.length; ++j) {
			if (PlayerHandler.players[j] != null) {
				...
				if (c.inPcBoat()) {
					...
					// c.sendMessage("Next Departure: " +
					// this.secondsToMinutesAndSeconds(this.waitTimer));
	...
	public void setInterface() {
		for (int j = 0; j < Server.playerHandler.players.length; j++) {
			if (Server.playerHandler.players[j] != null) {
				/*
				 * if (Server.playerHandler.players[j].inPcBoat()) { Client c = (Client)
			...
			if (Server.playerHandler.players[j] != null) {
				if (Server.playerHandler.players[j].inPcGame()) {
					...
					if (won && c.pcDamage > 10) {
						c.sendMessage("<shad=6081134>You have won the Minigame and won 4 Torva-Ps Points!");
						c.pcPoints += 4;
