...
public class Lottery {
	...
	public int checkEntriesCount(Client c) {
		...
		for (int indexes = 0; indexes < lotteryPlayersNames.size(); indexes++) {
			System.out.println(lotteryPlayersNames.get(indexes));
		...
		try {
			...
			for (int j = 0; j < PlayerHandler.players.length; j++) {
				if (PlayerHandler.players[j] != null) {
					...
					all.sendMessage("<shad=40960>[Lottery]<shad=-1> The Lottery has been won by "
	...
	public void announceFund() {
		...
		for (int j = 0; j < PlayerHandler.players.length; j++) {
			if (PlayerHandler.players[j] != null) {
				Client all = (Client) PlayerHandler.players[j];
				all.sendMessage("<shad=40960>[Lottery]<shad=-1> The Lottery Fund is currently at " + fund
						+ "m. Enter by talking to lottie at Edgeville");
			}
		}
	}
