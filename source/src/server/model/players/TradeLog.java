...
public class TradeLog {
	...
	/**
	 * Will write what kind of item a player has received. MONTH = 0 = January DAY
	 * OF MONTH = 30 || 31
	 */
	public void tradeReceived(String itemName, int itemAmount) {
		Client o = (Client) PlayerHandler.getPlayers()[c.tradeWith];
		Calendar C = Calendar.getInstance();
		try {
			BufferedWriter bItem = new BufferedWriter(
					new FileWriter("./data/Logs/TradeLogs/received/" + c.playerName + ".txt", true));
			try {
				bItem.newLine();
	...
	/**
	 * Will write what kind of item a player has traded with another player. MONTH =
	 * 0 = January DAY OF MONTH = 30 || 31
	 */
	public void tradeGive(String itemName, int itemAmount) {
		...
		try {
			BufferedWriter bItem = new BufferedWriter(
					new FileWriter("./data/Logs/TradeLogs/gave/" + c.playerName + ".txt", true));
			try {
				bItem.newLine();
				bItem.write("Year : " + C.get(Calendar.YEAR) + "\tMonth : " + C.get(Calendar.MONTH) + "\tDay : "
						+ C.get(Calendar.DAY_OF_MONTH));
