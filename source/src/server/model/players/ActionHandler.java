...
case 13670:
		...
		case 6462:
			c.getPA().movePlayer(3204, 3472, 0);
		break;
		...
		case 2423:
			if (c.getItems().playerHasItem(995, 250000000)) {
				...
				c.sendMessage("Teleport Home Now If You Are Not Prepared.");
		...
		case 2472:
		if (c.issDonator == 1){
			c.getPA().movePlayer(1889, 4958, 2);
			c.sendMessage("You teleport to the Mega-Boss Area.");
		} else {
			c.sendMessage("You must be a Mega Donator to use this portal!");
		}
		break;
		...
		case 2111:
			if (c.playerLevel[14] >= 125) { 
				if (System.currentTimeMillis() - c.buryDelay > 7500) {
					if (c.playerEquipment[c.playerWeapon] == 1275){
						c.startAnimation(6752);
						c.sendMessage("You mine some Dragonbone ore.");
...
case 13641:
		...
		case 104:
			if (c.getItems().playerHasItem(14496,1) && c.getItems().freeSlots() >= 1 && System.currentTimeMillis() - c.buryDelay > 1500) {
				c.getItems().deleteItem(14496, 1);
				c.getItems().addItem(c.getPA().randomCrystals(), 1);
				c.getDH().sendDialogues(38, 945);
				c.buryDelay = System.currentTimeMillis();
			} else {
				return; 
...
break;
	...
	c.fishitem = -1;
		...
		{
		case 219:
			c.getPA().startTeleport2(2603, 3414, 0); // What are the coords you want?
			break;
			case 1337:
...
break;
		...
		// end of hunter :)
			...
			case 519:
				...
				c.getPA().startTeleport2(3046, 9770, 0); // Done
			break;
			case 1282:
...
break;*/
		...
		break;
			...
			case 519:
				c.getShops().openShop(8); // Lol, it does it alredy
			break;
