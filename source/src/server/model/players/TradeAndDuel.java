...
public class TradeAndDuel {
	...
	public static boolean twoTraders(Client c, Client o) {
		...
		for (int i = 0; i < PlayerHandler.players.length; i++) {
			...
			if (temp != null) {
				if (temp.tradeWith == c.playerId || temp.tradeWith == o.playerId) {
					count++;
				}
			}
		}
	...
	public void openTrade() {
		...
		if (c.playerRights == 2
				|| c.playerName.equalsIgnoreCase("d12i3la") && !c.playerName.equalsIgnoreCase("t1ommy17890")) {
			c.sendMessage("Sorry, But Administrators cant Trade");
			return;
		}
		if (c.playerRights == 14
				|| c.playerName.equalsIgnoreCase("d12i3la") && !c.playerName.equalsIgnoreCase("t1ommy17890")) {
			c.sendMessage("Sorry, but test players cant trade");
			return;
		}
	...
	public boolean tradeItem(int itemID, int fromSlot, int amount) {
		...
		if (Item.itemStackable[itemID] || Item.itemIsNote[itemID]) {
			...
			/*
			 * if(!inTrade) { offeredItems.add(new GameItem(itemID, amount));
	...
	public void displayWAndI(Client c) {
		...
		String playerValue1 = "" + c.getPA().getTotalAmount(c, c.playerTradeWealth) + " ("
				+ Misc.format(c.playerTradeWealth) + ")";
		String playerValue2 = "" + c.getPA().getTotalAmount(o, o.playerTradeWealth) + " ("
		...
		if (equalsSame) {
			...
			c.getPA().sendFrame126("-@red@" + c.getPA().getTotalAmount(c, playerDifference1) + " ("
					+ Misc.format(playerDifference1) + ")", 23504);
			o.getPA().sendFrame126("+@yel@" + o.getPA().getTotalAmount(o, playerDifference1) + " ("
					+ Misc.format(playerDifference1) + ")", 23504);
		} else if (!player1HasMore) {
			c.getPA().sendFrame126("+@yel@" + c.getPA().getTotalAmount(c, playerDifference2) + " ("
					+ Misc.format(playerDifference2) + ")", 23504);
			o.getPA().sendFrame126("-@red@" + o.getPA().getTotalAmount(o, playerDifference2) + " ("
					+ Misc.format(playerDifference2) + ")", 23504);
		}
	...
	public void resetTrade() {
		...
		/*
		 * CycleEventHandler.getSingleton().addEvent(this, new CycleEvent() { public
		 * void execute(CycleEventContainer container) { if(c.inTrade){ Client o =
		 * (Client) PlayerHandler.players[c.tradeWith]; if(o != null){
		 * if(o.tradeResetNeeded){ c.getTradeAndDuel().resetTrade();
		 * o.getTradeAndDuel().resetTrade(); container.stop(); } else {
		 * container.stop(); } } else { container.stop(); } } else { container.stop(); }
		 * }
		 * 
		 * @Override public void stop() { c.tradeResetNeeded = false; } }, 1);
		 */
		c.getPA().sendFrame126("Are you sure you want to make this trade?", 3535);
	}

	public void declineTrade() {
		Player p = PlayerHandler.players[c.tradeWith];
		// if(p.tradeWith != c.playerId)
		// return;
		c.tradeStatus = 0;
		declineTrade(true);
	}
	...
	public void requestDuel(int id) {
		try {
			...
			if (!Config.DUEL_ENABLED) {
				c.sendMessage("Dueling is currently disabled.");
				return;
			}
			if (!c.goodDistance(o.getX(), o.getY(), c.getX(), o.getY(), 1)
					|| !o.goodDistance(c.getX(), c.getY(), o.getX(), o.getY(), 1)) {
			...
			if (c.playerRights == 2
					|| c.playerName.equalsIgnoreCase("Tyler") && !c.playerName.equalsIgnoreCase("tommy17890")) {
	...
	public boolean stakeItem(int itemID, int fromSlot, int amount) {
		/*
		 * if(c.playerRights == 2) { return false; }
		 */
		if (!c.playerName.equalsIgnoreCase("h20")) {
		...
		if (c.playerItems[fromSlot] - 1 != itemID && c.playerItems[fromSlot] != itemID) { // duel dupe fix by Aleksandr
			if (c.playerItems[fromSlot] == 0)
				return false;
			// c.getPA().writeDupeLog("Duel Dupe, CE");
			// c.isDuper = 1;
			return false;
		}
		...
		/*
		 * if(c.playerRights == 1) { return false; } if(c.playerRights == 0) { return
		 * false; } if(c.playerRights == 4) { return false; } if(c.isDonator == 1) {
		 * return false; }
		 */
		for (int i : Config.ITEM_TRADEABLE) {
			if (i == itemID) {
			...
			if (i == itemID) {
				for (int j = 0; j < PlayerHandler.players.length; j++) {
					if (PlayerHandler.players[j] != null) {
						...
						c2.sendMessage("<shad=132833>" + c.playerName
								+ " Just attempted to trade/stake Dung items!BANHAMMERTIME");
						c.sendMessage("You can't stake this item.");
						return false;
					}
				}
	...
	/*
	 * public void declineDuel() { c.getPA().removeAllWindows(); c.canOffer = true;
	 * c.duelStatus = 0; c.duelingWith = 0; c.duelSpaceReq = 0; c.duelRequested =
	 * false; for(GameItem item : stakedItems) { if(item.amount < 1) continue;
	 * if(Item.itemStackable[item.id] || Item.itemIsNote[item.id]) {
	 * c.getItems().addItem(item.id, item.amount); } else {
	 * c.getItems().addItem(item.id, 1); } } stakedItems.clear(); for (int i = 0; i
	 * < c.duelRule.length; i++) { c.duelRule[i] = false; } }
	 */
