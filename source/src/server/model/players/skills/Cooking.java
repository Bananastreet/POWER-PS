...
public class Cooking extends SkillHandler {
	...
	private static void cookFish(Client c, int itemID, int xpRecieved, int levelRequired, int burntFish, int cookedFish,
			int object) {
		...
		Server.getTaskScheduler().addEvent(new Task(2, false) {
			public void execute() {
				...
				if (c.playerLevel[7] >= fishStopsBurning(c.playerSkillProp[7][0])
						|| Misc.random(c.playerSkillProp[7][6]) > Misc.random(c.playerSkillProp[7][2])) {
					c.sendMessage("You successfully cook the "
							+ ItemAssistant.getItemName(c.playerSkillProp[7][0]).toLowerCase() + ".");
