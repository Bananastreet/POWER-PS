...
public class Firemaking {
	...
	public static void grabData(final Client c, final int useWith, final int withUse) {
		...
		/*
		 * if(!c.inFM()) {
		 * c.sendMessage("You can only create fires in the FM skilling-area"); return; }
		 */
		for (int i = 0; i < data.length; i++) {
			if ((requiredItem(c, useWith) && withUse == data[i][0]
					|| useWith == data[i][0] && requiredItem(c, withUse))) {
				...
				if (System.currentTimeMillis() - c.lastFire > 1200) {
					...
					// Server.itemHandler.createGroundItem(c, log, coords[0], coords[1], 1,
					// c.getId());
					...
					Server.getTaskScheduler().addEvent(new Task(20, false) {
						public void execute() {
							...
							// Server.itemHandler.createGroundItem(c, 592, coords[0], coords[1], 1,
							// c.getId());
							this.stop();
						}
