...
public class Dungeoneering {
	...
	// public int[] startItems = new int[]
	// {1725,1067,1115,5698,4587,10828,7462,6568,3105,8850};
		...
		Server.getTaskScheduler().addEvent(new Task(1, false) {
			public void execute() {
				if (!hasSpawnedNPCswave2) {
					...
					c.juststarted = false;
					// Server.npcHandler.spawnDungNpcs(c, 4477, 3255, 9333, 4, 0, 140, 24, 240, 230,
					// true, false); // mad mummy
	...
	public static void handleChaosEleTeleporting(final Client c) {
		Server.getTaskScheduler().addEvent(new Task(8, false) {
			public void execute() {
				...
				c.sendMessage(
						"<shad=6081134>[Dungeoneering] <shad=15695415>The Chaos Elemental teleports you into the middle!");
				this.stop();
				if (c.disconnected) {
					this.stop();
					return;
	...
	public static void startfloor1(Client c) {
		/*
		 * for (int j = 0; j < c.playerItems.length; j++) { if(c.playerItems[j] > 0) {
		 * c.sendMessage("Bank all your items first!!"); c.getPA().closeAllWindows();
		 * return; }
		 */
		...
		if (!hasSentMessage == true) {
			...
			c.sendMessage(
					"<shad=6081134>[Dungeoneering] <shad=15695415>You've entered the Lobby! Please customize your character..");
			c.sendMessage(
					"<shad=6081134>[Dungeoneering] <shad=15695415>To start Dungeoneering, enter the door to the north!");
			hasSentMessage = true;
			c.dungn = 0;
		}
	}
	...
	public void handleReward(Client c) {
		...
		c.sendMessage(
				"<shad=6081134>[Dungeoneering] <shad=15695415>All waves completed! Reward; 100 Tokens and 150K Dungeoneering XP!");
	...
	public static void handleObject(Client c, int id, int x, int y) {
		...
		case 6553:
			...
			if (c.absX == 3246 && c.absY == 9333 && c.dungn == 5) { // third door
				...
				if (c.absX == 3246 && c.absY == 9333 && c.dungn != 5) {
					c.sendMessage(
							"<shad=6081134>[Dungeoneering] <shad=15695415>You need to have atleast 5 kills to enter there!");
				}
			}
			...
			if (c.absX == 3260 && c.absY == 9329 && c.dungn == 4) { // second door
				...
				if (c.absX == 3260 && c.absY == 9329 && c.dungn != 4) {
					c.sendMessage(
							"<shad=6081134>[Dungeoneering] <shad=15695415>You need to have atleast 4 kills to enter there!");
				}
			}
			...
			if (c.absX == 3219 && c.absY == 9328 && c.dungn == 7 || c.absX == 3219 && c.absY == 9328 && c.dungn == 8) { // third
																														// door
				c.getPA().movePlayer(3217, 9328, c.playerId * 4);
				spawnDungNpcWave4(c);
			} else {
				if (c.absX == 3219 && c.absY == 9328 && c.dungn != 7
						|| c.absX == 3219 && c.absY == 9328 && c.dungn != 7) {
					c.sendMessage(
							"<shad=6081134>[Dungeoneering] <shad=15695415>You need to have atleast 7 kills to enter there!");
				}
			}
			if (c.absX == 3219 && c.absY == 9327 && c.dungn == 7 || c.absX == 3219 && c.absY == 9327 && c.dungn == 8) { // third
																														// door
				c.getPA().movePlayer(3217, 9327, c.playerId * 4);
				spawnDungNpcWave4(c);
			} else {
				if (c.absX == 3219 && c.absY == 9327 && c.dungn != 7
						|| c.absX == 3219 && c.absY == 9327 && c.dungn != 8) {
					c.sendMessage(
							"<shad=6081134>[Dungeoneering] <shad=15695415>You need to have atleast 7 kills to enter there!");
				}
			}
		...
		case 6555:
			if (!c.InDung()) {
				c.sendMessage(
						"<shad=6081134>[Dungeoneering] <shad=15695415>You cannot use theses objects, teleport home and re-do the Dungeon.");
				return;
