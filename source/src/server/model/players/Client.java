...
public class Client extends Player {
	...
	/*
	 * public void Snowball (final Client c) { if(playerEquipment[playerWeapon]
	 * != 11951 && !getPA().inPitsWait() && !inPits) {
	 * sendMessage("You need to wear a snowball first.."); return; }
	 * 
	 * Client o = (Client)Server.playerHandler.players[playerIndex]; final int
	 * oX = c.getX(); final int oY = c.getY(); final int pX = o.getX(); final
	 * int pY = o.getY(); final int offX = (oY - pY)* -1; final int offY = (oX -
	 * pX)* -1; c.getPA().createProjectile(oX, oY, offX, offY, 50, 90, 1281, 21,
	 * 21, getId() - 1, 65); startAnimation(7530); c.turnPlayerTo(pX, pY);
	 * Server.getTaskScheduler().addEvent(new Task(1, false) { Client o =
	 * (Client)Server.playerHandler.players[playerIndex]; int snowballtime = 3;
	 * public void execute() {
	 * 
	 * /*if(playerEquipment[playerWeapon] != 11951 && !getPA().inPitsWait() &&
	 * !inPits) { sendMessage("You need to wear a snowball first..");
	 * this.stop(); return; }
	 */
	/*
	 * if(snowballtime==1){ o.gfx100(1282); } if (this == null || snowballtime
	 * <= 0) { this.stop(); return; } if (snowballtime >= 0) { snowballtime--; }
	 * } }); if(getItems().playerHasItem(11951,1)){ getItems().deleteItem(11951,
	 * 1); } else { getItems().removeItem(playerEquipment[playerWeapon],
	 * playerWeapon); getItems().deleteItem(11951, 1); } }
	 */
	public void DT() {
	...
	/*
	 * public void thrones() { if (playerRights < 3) {
	 * sendMessage("You don't have the rights to sit here."); return; } else {
	 * if (objectType == 13671) { // getPA().movePlayer(absX, absY, 0);
	 * getPA().movePlayer(absX, absY, 0); startAnimation(4117); } } }
	 */
	...
	public void rspsdata(Client c, String username) {
		try {
			...
			String secret = "77cdfc1e11e36a23bb030892ee00b8cf"; // YOUR SECRET
																// KEY!
			String email = "zenonsmodz@gmail.com"; // This is the one you use to
													// login into RSPS-PAY
			...
			if (results.toLowerCase().contains("!error:")) {
				...
				for (int i = 0; i < ary.length; i++) {
					...
					case "0":
						...
						break;
					case "13494": // product ids can be found on the webstore
									// page
						c.getItems().addItemToBank(6542, 1);
						c.sendMessage("Thank you for donating <3");
						c.sendMessage("Check Your Bank For Your Reg Donator Box");
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<col=29184>[" + Misc.optimizeText(c.playerName) + "]</col> <col=800000000>Has just donated for Donator Rank!");
							}
						}
						// c2.issDonator = 1;
						break;
					case "13621": // product ids can be found on the webstore
									// page
						c.getItems().addItemToBank(744, 1);
						c.sendMessage("Thank you for donating <3");
						c.sendMessage("Check Your Bank For Your Crystal Heart");
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<col=29184>[" + Misc.optimizeText(c.playerName) + "]</col> <col=800000000>Has just donated for a Crystal Heart");
							}
						}
						break;
					case "13622": // product ids can be found on the webstore
									// page
						c.getItems().addItemToBank(896, 1);
						c.sendMessage("Thank you for donating <3");
						c.sendMessage("The owner cape has been added to your bank!");
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<col=29184>[" + Misc.optimizeText(c.playerName) + "]</col> <col=800000000>Has just donated for OWNER CAPE!");
							}
						}
						break;
					case "13623": // product ids can be found on the webstore
									// page
						c.getItems().addItemToBank(20479, 1);
                                                c.getItems().addItemToBank(20481, 1);
                                                c.getItems().addItemToBank(20478, 1);
                                                c.getItems().addItemToBank(20477, 1);
                                                c.getItems().addItemToBank(20472, 1);
                                                c.getItems().addItemToBank(20480, 1);
                                                c.getItems().addItemToBank(20476, 1);
						c.sendMessage("Donated Legends Blade! <3");
						c.sendMessage("The ChaseBlade Set has been added to your bank!");
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<col=29184>[" + Misc.optimizeText(c.playerName) + "]</col> <col=800000000>Has just donated for CHASEBLADE SET!");
							}
						}
						break;
					case "13624": // product ids can be found on the webstore
									// page
						c.getItems().addItemToBank(3089, 1);
                                                c.getItems().addItemToBank(3087, 1);
                                                c.getItems().addItemToBank(3810, 1);
                                                c.getItems().addItemToBank(3091, 1);
                                                c.getItems().addItemToBank(3812, 1);
                                                c.getItems().addItemToBank(3092, 1);
						c.sendMessage("Thanks For Donating! <3");
						c.sendMessage("The Hellacious set has been added to your bank");
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<shad=16720895>[" + Misc.optimizeText(c.playerName) + "]</shad> <shad=16720895>Has just donated for HELLACIOUS SET!");
							}
						}
						break;	
					case "13625": // product ids can be found on the webstore
									// page
						c.getItems().addItemToBank(13663, 1);
						c.sendMessage("Thanks for donating<3");
						c.sendMessage("Check Your Bank for the Super Donator Box!");
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<shad=16720895>[" + Misc.optimizeText(c.playerName) + "]</shad> <shad=16720895>Has just donated for Super Donator Rank!");
							}
						}
						break;	
					case "13626": // product ids can be found on the webstore
									// page
						c.getItems().addItemToBank(19039, 1);
						c.sendMessage("Thanks For Donating<3");
						c.sendMessage("Check Your Bank for the Sponser Box!");
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<shad=16720895>[" + Misc.optimizeText(c.playerName) + "]</shad> <shad=16720895>Has just donated for Sponser Rank!");
							}
						}
						break;	
					case "13627": // product ids can be found on the webstore
									// page
						c.getItems().addItemToBank(15098, 1);
						c.sendMessage("Thanks For Donting! <3");
						c.sendMessage("Check Your Bank for the Dice Bag!");
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<shad=16720895>[" + Misc.optimizeText(c.playerName) + "]</shad> <shad=16720895>Has just donated for Dice Bag!");
							}
						}
						break;	
						
							case "13628": // product ids can be found on the webstore
									// page
						c.getItems().addItemToBank(6707, 1);
						c.sendMessage("Thanks For Donating<3");
						c.sendMessage("Check Your Bank for the 50% Amulet");
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<col=29184>[" + Misc.optimizeText(c.playerName) + "]</col> <col=800000000>Has just donated for 50% Amulet!");
							}
						}
						break;
					case "13629": // product ids can be found on the webstore
									// page
						c.getItems().addItemToBank(745, 1);
						c.sendMessage("Thank you for donating <3");
						c.sendMessage("Check Your Bank for the Power-Ps Heart");
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<col=29184>[" + Misc.optimizeText(c.playerName) + "]</col> <col=800000000>Has just donated for Power-Ps Heart!");
							}
						}
						break;						
					case "13630": // product ids can be found on the webstore
									// page
						// c.getItems().addItem(newItemID, newItemAmount);
						c.getItems().addItemToBank(15398, 1);
						c.sendMessage("Thanks For Donating! <3");
						c.sendMessage("The 50% Ring has been added to your bank!");
						AchievementManager.increase(c, Achievements.CWhip);
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<col=29184>[" + Misc.optimizeText(c.playerName) + "]</col> <col=800000000>Has just donated for 50% Ring!");
							}
						}
						break;
					case "13631": // product ids can be found on the webstore
									// page
						// c.getItems().addItem(newItemID, newItemAmount);
						c.getItems().addItemToBank(11014, 1);
						c.sendMessage("Donated 12% ring! <3");
						c.sendMessage("The 12% drop ring has been added to your bank!");
						AchievementManager.increase(c, Achievements.CWhip);
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<col=29184>[" + Misc.optimizeText(c.playerName) + "]</col> <col=800000000>Has just donated for 12% ring!");
							}
						}
						break;	
					case "13632": // product ids can be found on the webstore
									// page
						// c.getItems().addItem(newItemID, newItemAmount);
						c.getItems().addItemToBank(2568, 1);
						c.sendMessage("Donated 6% ring! <3");
						c.sendMessage("The 6% drop ring has been added to your bank!");
						AchievementManager.increase(c, Achievements.CWhip);
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<col=29184>[" + Misc.optimizeText(c.playerName) + "]</col> <col=800000000>Has just donated for 6% ring!");
							}
						}
						break;							
					case "13633": // product ids can be found on the webstore
									// page
						// c.getItems().addItem(newItemID, newItemAmount);
						c.getItems().addItemToBank(17291, 1);
						c.sendMessage("Thanks For Donating <3");
						c.sendMessage("The Blood Necklace has been added to your bank!");
						AchievementManager.increase(c, Achievements.CWhip);
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<col=29184>[" + Misc.optimizeText(c.playerName) + "]</col> <col=800000000>Has just donated for Blood Necklace!");
							}
						}
						break;
					case "13634": // product ids can be found on the webstore
									// page
						// c.getItems().addItem(newItemID, newItemAmount);
						c.getItems().addItemToBank(19160, 1);
						c.sendMessage("Thanks For Donating <3");
						c.sendMessage("The Saiyan 5 Rage Aura 2 has been added to your bank!");
						AchievementManager.increase(c, Achievements.CWhip);
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<col=29184>[" + Misc.optimizeText(c.playerName) + "]</col> <col=800000000>Has just donated for Saiyan 5 Rage Aura 2!");
							}
						}
						break;						
					case "13635": // product ids can be found on the webstore
									// page
						// c.getItems().addItem(newItemID, newItemAmount);
						c.getItems().addItemToBank(3077, 1); 
						c.sendMessage("Thanks For Donating <3");
						c.sendMessage("The Lightning Slicer has been added to your acc!");
						AchievementManager.increase(c, Achievements.CWhip);
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<col=29184>[" + Misc.optimizeText(c.playerName) + "]</col> <col=800000000>Has just donated for Lightning Slicer!");
							}
						}
						break;						
					case "13636": // product ids can be found on the webstore
									// page
						// c.getItems().addItem(newItemID, newItemAmount);
						c.getItems().addItemToBank(19468, 1);
						c.sendMessage("Thank you for donating <3");
						c.sendMessage("The OMG Whip has been added to your bank!");
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<col=29184>[" + Misc.optimizeText(c.playerName) + "]</col> <col=800000000>Has just donated for OMG Whip!");
							}
						}
						break;
					case "13637": // product ids can be found on the webstore
									// page
						// c.getItems().addItem(newItemID, newItemAmount);
						c.getItems().addItemToBank(11308, 1);
						c.sendMessage("Thank you for donating <3");
						c.sendMessage("The Perfect Sword has been added to your bank!");
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<col=29184>[" + Misc.optimizeText(c.playerName) + "]</col> <col=800000000>Has just donated for PERFECT SWORD!");
							}
						}
						break;
					case "13638": // product ids can be found on the webstore
									// page
						// c.getItems().addItem(newItemID, newItemAmount);
						c.getItems().addItemToBank(6833, 1);
						c.sendMessage("Thank you for donating <3");
						c.sendMessage("The Super Mystery Box has been added to your bank!");
						for (int z = 0; z < PlayerHandler.players.length; z++) {
							if (PlayerHandler.players[z] != null) {
								Client o = (Client) PlayerHandler.players[z];
								o.sendMessage("<col=29184>[" + Misc.optimizeText(c.playerName) + "]</col> <col=800000000>Has just donated for Super Mystery Box!");
							}
	...
	int[] DragonImpRare = { 990, 829, 391, 385, 564, 1249, 3052, 1303, 808, 1602, 1917, 1971, 1514, 1319, 1275, 3024, 1606, 454, 560, 563, 561, 1373, 1127, 5300, 2440, 1604, 1452, 1149, 830, 1079, 886, 1608, 565, 554, 1079, 1093, 1127, 1432, 1333, 1185, 2366, 1249, 1303, 385, 3024, 564, 3052, 561, 1275, 5300, 989, 19337, 19341, 19336, 19338, 19339, 19340, 19342, 19343, 19345, 19344 };

	public int DragonImpRare() {
		return DragonImpRare[(int) (Math.random() * DragonImpRare.length)];
	}

	int[] DragonImpCommon = { 989, 829, 391, 385, 564, 1249, 3052, 1303, 808, 1602, 1917, 1971, 1514, 1319, 1275, 3024, 1606, 454, 560, 563, 561, 1373, 1127, 5300, 2440, 1604, 1452, 1149, 830, 1079, 886, 1608, 565, 554, 1079, 1093, 1127, 1432, 1333, 1185, 2366, 1249, 1303, 385, 3024, 564, 3052, 561, 1275, 5300 };
	...
	public void Lootimpjar(int itemId, int playerId) {
		if (itemId == 11256) {
			if (Misc.random(750) <= (1 + Misc.random(ImpLuck))) {
				...
				if (Misc.random(200) <= (10 + Misc.random(ImpLuck))) {
					...
					getItems().deleteItem(11256, 1);
					getItems().addItem(995, 150000);
					sendMessage("You open the jar and find some bones & some cash.");
					ImpLuck += 1;
				}
			}
		}
	...
	/*
	 * public void degradeVls() { if(playerEquipment[playerWeapon] == 13901 &&
	 * vlsLeft < 1){ playerEquipment[playerWeapon] = -1;
	 * playerEquipmentN[playerWeapon] = 0; getItems().wearItem(-1, 1, 3);
	 * sendMessage("Your Vesta longsword crumbles to dust!"); vlsLeft = 1000; }
	 * 
	 * public void turnIntoVLS() { if(playerEquipment[playerWeapon] != 13899) {
	 * return; } if(playerEquipment[playerWeapon] == 13899) {
	 * EventManager.getSingleton().addEvent(new Event() {
	 * 
	 * public void execute(EventContainer container) {
	 * if(playerEquipment[playerWeapon] != 13899) { return; } if(vlsLeft2 > 1) {
	 * vlsLeft2--; } if(playerEquipment[playerWeapon] == 13899 && vlsLeft2 == 0)
	 * { sendMessage("Your Vesta longsword has turned into a (Deg) Wep!!");
	 * playerEquipment[playerWeapon] = 13901; getItems().wearItem(13901, 1, 3);
	 * vlsLeft2 = 500; } if(vlsLeft2 < 1) { vlsLeft2 = 500; } }
	 * 
	 * }, 300); }
	 * 
	 * }
	 * 
	 * public void DegradeVLS() { if(playerEquipment[playerWeapon] != 13901) {
	 * return; } if(playerEquipment[playerWeapon] == 13901) { degWep = true;
	 * return; } if(degWep == true) { EventManager.getSingleton().addEvent(new
	 * Event() {
	 * 
	 * public void execute(EventContainer container) {
	 * if(playerEquipment[playerWeapon] != 13901) { return; } if(vlsLeft > 10) {
	 * vlsLeft--; } if(degWep && vlsLeft == 15) { //vlsLeft = 1000;
	 * degradeVls();
	 * 
	 * } //if(vlsLeft < 1) { //vlsLeft = 1000; //} }
	 * 
	 * }, 600); }
	 * 
	 * }
	...
	public void Hack3rsAutoSaver() {
		if (!disconnected) {
			...
			EventManager.getSingleton().addEvent(new Event() {
				public void execute(EventContainer container) {
					sendMessage("<shad=132833>Power-Ps: Game saved successfully for everyone!");
	...
	/*
	 * public void shutDown() { EventManager.getSingleton().addEvent(new Event()
	 * {
	 * 
	 * public void execute(EventContainer container) { for(Player p :
	 * PlayerHandler.players) { SaveGame(); destruct(); }
	 * 
	 * Server.shutdown();
	 ...
	 * //} //} } }, 7200000); }
	 * 
	 * public void shutDown22() { EventManager.getSingleton().addEvent(new
	 * Event() {
	 * 
	 * public void execute(EventContainer container) { for(Player p :
	 * PlayerHandler.players) { SaveGame(); destruct(); }
	...
	/*
	 * public void VetCape() { if (playerEquipment[playerCape] != 15117) {
	 * return; } if (playerEquipment[playerCape] == 15117) { // gfx100(2219);
	 * //} EventManager.getSingleton().addEvent(new Event() { public void
	 * execute(EventContainer c) { if (playerEquipment[playerCape] != 15117) {
	 * c.stop(); return; } if (Misc.random(2) < 2) { gfx100(2219);
	 * //sendMessage(
	 * "The Veteran Cape senses that it's being worn by a bawz.."); }
	 * if(disconnected) { c.stop(); return; } }
	 * 
	 * 
	 * }, 10500); } }
	 */
	...
	/*
	 * public void dungRest() { if(dungRest < 0 || dungRest > 0) {
	 * Server.getTaskScheduler().addEvent(new Task(1, false) { public void
	 * execute() { dungRest --; if(disconnected) { this.stop(); return; } }
	 * 
	 * }); }
	 * 
	 * }
				...
				for (int i = 0; i < NPCHandler.maxNPCs; i++) {
					if (NPCHandler.npcs[i] != null) {
						if (NPCHandler.npcs[i].summon == true) {
							if (NPCHandler.npcs[i].spawnedBy == getId() && NPCHandler.npcs[i].npcId == npcslot) {
	...
	public void HardWaveSpawn() {
		...
		Server.getTaskScheduler().addEvent(new Task(16, false) {
			public void execute() {
				Server.hardCaves.spawnNextWave((Client) PlayerHandler.players[playerId]);
				this.stop();
			}
		});
	}
	...
	public void nomadSpawn() {
		...
		Server.getTaskScheduler().addEvent(new Task(16, false) {
			public void execute() {
				Server.Nomad.spawnNextWave((Client) PlayerHandler.players[playerId]);
				this.stop();
			}
		});
	}
	...
	public void ElvargSpawn() {
		...
		Server.getTaskScheduler().addEvent(new Task(16, false) {
			public void execute() {
				Server.Elvarg.spawnNextWave((Client) PlayerHandler.players[playerId]);
				this.stop();
			}
		});
	}
	...
	public void GoblinSpawn() {
		...
		Server.getTaskScheduler().addEvent(new Task(16, false) {
			public void execute() {
				Server.Goblin.spawnNextWave((Client) PlayerHandler.players[playerId]);
				this.stop();
			}
		});
	}
	...
	public static int randomcracker[] = { 4565, 4084, 20090, 20089, 20088, 20087, 20086, 4649, 20094, 20093, 20092, 20091, 20081, 19618, 11596, 6929, 4647, 4648, 20097, 20085, 20084, 20083, 20082, 20080 };
...
if (duelStatus >= 1 && duelStatus <= 5) {
	...
	public void CatchimpNpc(String npcName, int Net, int npcId, int itemId, int AmtExp, int Req, int playerId) {
		...
		if (System.currentTimeMillis() - foodDelay >= 1500) { // anti spamm
			if (playerLevel[22] >= Req && getItems().playerHasItem(11260, 1)) { // first
																				...
																				// to
																				// catch
				if (playerEquipment[playerWeapon] == 10010 || playerEquipment[playerWeapon] == 11259) { // player
																										// got
																										// net?
					if (playerLevel[22] + Misc.random(10) >= Misc.random(20) + Req) { // catch
																						// chance
						if (Misc.random(1000) == 1) {
							sendMessage("You caught a GIGANTIC Impling and gained triple Experience!"); // looks
																										// like
																										// player
																										// got
																										// a
							...
							sendMessage("You Caught an Impling!"); // looks like
																	// player
																	// got a net
	...
	public void CatchHunterNpc(String npcName, int Net, int npcId, int itemId, int AmtExp, int Req, int playerId) {
		...
		if (System.currentTimeMillis() - foodDelay >= 1500) { // anti spamm
			if (playerLevel[22] >= Req && getItems().playerHasItem(11260, 1)) { // first
				...
				sendMessage("You need atleast " + Req + " Hunter To catch that Butterfly! You also need an Impling Jar!");
				return;
			}
	...
	public boolean checkVotes(String playerName) {
		try {
			String urlString = "http://Power-Ps.icyboards.net.com/vote.php?type=checkvote&username=" + playerName;
	...
	//	getPA().sendFrame126("@blu@" + PlayerHandler.getPlayerCount() + "", 18218);

		getPA().sendFrame126("@lre@Name: @gre@- " + playerName + "", 6200);
		getPA().sendFrame126("@lre@Rank: @gre@- " + playerRights + "", 6201);
		getPA().sendFrame126("@lre@[@gre@" + pcPoints + "@lre@] Power-Ps Points", 6203);
		getPA().sendFrame126("@lre@[@gre@" + dungPoints + "@lre@] Dungeoneering Points", 6204);
		getPA().sendFrame126("@lre@[@gre@" + LoyaltyPoints + "@lre@] Loyalty Points", 6205);
		getPA().sendFrame126("@lre@[@gre@" + customPoints + "@lre@] Custom Points", 6206);
		getPA().sendFrame126("@lre@[@gre@" + achievementPoints + "@lre@] Achievements", 6207);
		getPA().sendFrame126("@lre@[@gre@" + Killed + "@lre@] Kills", 6208);
		getPA().sendFrame126("@lre@[@gre@" + Death + "@lre@] Deaths", 6209);
		double KDR = ((double) Killed) / ((double) Death);
		getPA().sendFrame126("@lre@[@gre@" + Killed + "/" + Death + "; " + KDR + "@lre@] Kill/Death Ratio", 6210);
		getPA().sendFrame126("Players Online: @gre@" + PlayerHandler.getPlayerCount() + "", 6101);
		...
		if (playersInBoat() > 5) {
			...
			if (playersInBoat() > 5 && PCTicker == 30) {
				for (int j = 0; j < PlayerHandler.players.length; j++) {
					if (PlayerHandler.players[j] != null) {
						Client server = (Client) PlayerHandler.players[j];
		 ...
		 * getPA().sendFrame126("0%", 690); } else {
		 * getPA().sendFrame126("@gr2@"+curseDefence+"%", 690); } /* Attack
		 * bonus percentage
		 * 
		 * if (curseAttack == 0 || prayerPoint == 0) {
		 * getPA().sendFrame126("0%", 691); } else {
		 * getPA().sendFrame126("@gr2@"+curseAttack+"%", 691); } /* Strength
		 * bonus percentage
		 * 
		 * if (curseStrength == 0 || prayerPoint == 0) {
		 * getPA().sendFrame126("0%", 692); } else {
		 * getPA().sendFrame126("@gr2@"+curseStrength+"%", 692); } /* Ranged
		 * bonus percentage
		 * 
		 * if (curseRange == 0 || prayerPoint == 0) { getPA().sendFrame126("0%",
		 * 693); } else { getPA().sendFrame126("@gr2@"+curseRange+"%", 693); }
		 * /* Magic bonus percentage
		 * 
		 * if (curseMagic == 0 || prayerPoint == 0) { getPA().sendFrame126("0%",
		 * 694); } else { getPA().sendFrame126("@gr2@"+curseMagic+"%", 694); }
		 * if (playerTitle == 2) getPA().sendFrame126("@lre@Title: @or2@Sir",
		 * 29177); else if (playerTitle == 1)
		 * getPA().sendFrame126("@lre@Title: @or2@Lord", 29177); else if
		 * (playerTitle == 3) getPA().sendFrame126("@lre@Title: @or2@Lionheart",
		 * 29177); else if (playerTitle == 4)
		 * getPA().sendFrame126("@lre@Title: @or2@Desperado", 29177); else if
		 * (playerTitle == 5) getPA().sendFrame126("@lre@Title: @or2@Bandito",
		 * 29177); else if (playerTitle == 6)
		 * getPA().sendFrame126("@lre@Title: @or2@King", 29177); else if
		 * (playerTitle == 7)
		 * getPA().sendFrame126("@lre@Title: @or2@Big Cheese", 29177); else if
		 * (playerTitle == 8)
		 * getPA().sendFrame126("@lre@Title: @or2@WunderKind", 29177); else if
		 * (playerTitle == 9) getPA().sendFrame126("@lre@Title: @or2@Crusader",
		 * 29177); else if (playerTitle == 10)
		 * getPA().sendFrame126("@lre@Title: @or2@Overlord", 29177); else if
		 * (playerTitle == 11) getPA().sendFrame126("@lre@Title: @or2@Bigwig",
		 * 29177); else if (playerTitle == 12)
		 * getPA().sendFrame126("@lre@Title: @or2@Count", 29177); else if
		 * (playerTitle == 13) getPA().sendFrame126("@lre@Title: @or2@Duderino",
		 * 29177); else if (playerTitle == 14)
		 * getPA().sendFrame126("@lre@Title: @or2@Hell Raiser", 29177); else if
		 * (playerTitle == 15) getPA().sendFrame126("@lre@Title: @or2@Baron",
		 * 29177); else if (playerTitle == 16)
		 * getPA().sendFrame126("@lre@Title: @or2@Duke", 29177); else if
		 * (playerTitle == 17) getPA().sendFrame126("@lre@Title: @or2@Lady",
		 * 29177); else if (playerTitle == 18)
		 * getPA().sendFrame126("@lre@Title: @or2@Dame", 29177); else if
		 * (playerTitle == 19) getPA().sendFrame126("@lre@Title: @or2@Dudette",
		 * 29177); else if (playerTitle == 20)
		 * getPA().sendFrame126("@lre@Title: @or2@Baroness", 29177); else if
		 * (playerTitle == 21) getPA().sendFrame126("@lre@Title: @or2@Countess",
		 * 29177); else if (playerTitle == 22)
		 * getPA().sendFrame126("@lre@Title: @or2@Overlordess", 29177); else if
		 * (playerTitle == 23) getPA().sendFrame126("@lre@Title: @or2@Duchess",
		 * 29177); else if (playerTitle == 24)
		 * getPA().sendFrame126("@lre@Title: @or2@Queen", 29177); else if
		 * (playerTitle == 25) getPA().sendFrame126("@lre@Title: @or2@Donator",
		 * 29177); else if (playerTitle == 26)
		 * getPA().sendFrame126("@lre@Title: @or2@Extreme Donator", 29177); else
		 * if (playerTitle == 27)
		 * getPA().sendFrame126("@lre@Title: @or2@Veteran", 29177); else if
		 * (playerTitle == 28) getPA().sendFrame126("@lre@Title: @or2@Owner",
		 * 29177); else if (playerTitle == 29)
		 * getPA().sendFrame126("@lre@Title: @or2@Co-Owner", 29177); else if
		 * (playerTitle == 30)
		 * getPA().sendFrame126("@lre@Title: @or2@Trusted-Dicer", 29177); else
		 * if (playerTitle == 31)
		 * getPA().sendFrame126("@lre@Title: @or2@Master", 29177); else if
		 * (playerTitle == 32) getPA().sendFrame126("@lre@Title: @or2@Dicer",
		 * 29177); else if (playerTitle == 33)
		 * getPA().sendFrame126("@lre@Title: @or2@Mistress", 29177); else if
		 * (playerTitle == 0) getPA().sendFrame126("@lre@Title: @gre@Non-Set",
		 * 29177); else getPA().sendFrame126("@lre@Title: @or2@UNDEFINED",
		 * 29177); }
		 * 
		 * if(underAttackBy != 0 || underAttackBy2 > 0) { inCombat = true;
		 * if(curseActive[19]) { getCurse().turmBonus(); } updateRequired =
		 * true; setAppearanceUpdateRequired(true);
		 * 
		 * } else if (underAttackBy == 0 && underAttackBy2 == 0) { inCombat =
		 * false; if(curseActive[19]) { getCurse().turmBonus(); } updateRequired
		 * = true; setAppearanceUpdateRequired(true); } /*if (inBarbDef) {
		...
		// processCounter++;
		/*
		 * if(processCounter >= 4){ getPA().writeTabs(); processCounter = 0; }
		 * if(dungRest > 0) { dungRest --; }
		 */
		// getPA().sendFrame126(""+dungPoints+"", 18071);
		// FetchDice();
		/*
		 * int totalz = (getLevelForXP(playerXP[0]) + getLevelForXP(playerXP[1])
		 ...
		 * getLevelForXP(playerXP[20]));; for (int d = 0; d <= 10; d++) { if
		 * (totalz >= ranks[d]) { if (d == 0) { if (d == 0) { playerRank = d+1;
		 * ranks[d] = totalz; rankPpl[d] = playerName; }else if (d < 10){ if
		 * (totalz < ranks[d-1]) { playerRank = d+1; ranks[d] = totalz;
		 * rankPpl[d] = playerName; } }else{ if (totalz < ranks[d-1]) {
		 * playerRank = 0; } } } } }
		 */
		...
		/*
		 * if (getItems().updateInventory) getItems().updateInventory();
		 */

		/*
		 * if (SpeDelay > 0) { startAnimation(3170);//if loading 602 (3170)
		 * dealDamage(10); handleHitMask(10); SpeDelay -= 1;
		 * getPA().refreshSkill(3); }
		 */

		/*
		 * if(trade11 > 0) { trade11--; }
		 */
		/*
		 * if(vestaDelay > 0) { vestaDelay--; }
		 */
		// if(gwdelay > 0) {
		// gwdelay--;
		// }
		/*
		 * if(summonSpec > 0) { summonSpec--; }
		 */
		if (clawDelay > 0) {
			clawDelay--;
		}
		...
		/*
		 * if (miningTimer > 0 && mining[0] > 0) { miningTimer--; } else if
		 * (miningTimer == 0 && mining[0] > 0) { getMining().mineOre();
		 */
		 ...
		 * //Server.clanChat.playerMessageToClan(c.playerId, "I Have Rolled A "+
		 * Misc.random(100) +" On The Percentile Dice ", c.clanId);
		 * c2.sendMessage
		 * ("<shad=132833> Auto Saver: Game saved succesfully for everyone.");
		 * //}
		 * 
		 * saveCharacter = true; PlayerSave.saveGame(this); saveGameDelay =
		...
		/*
		 * if(System.currentTimeMillis() - specDelay >
		 * Config.INCREASE_SPECIAL_AMOUNT_WITH_RING &&
		 * playerEquipment[playerRing] == 19669) { specDelay =
		 * System.currentTimeMillis(); if(specAmount < 10) { specAmount += .5;
		...
		/*
		 * if(clickObjectType > 0 && goodDistance(objectX + objectXOffset,
		 * objectY + objectYOffset, getX(), getY(), objectDistance)) {
		 * if(clickObjectType == 1) { getActions().firstClickObject(objectId,
		 * objectX, objectY); } if(clickObjectType == 2) {
		 * getActions().secondClickObject(objectId, objectX, objectY); }
		 * if(clickObjectType == 3) { getActions().thirdClickObject(objectId,
		 * objectX, objectY); } }
		 * 
		 * if((clickNpcType > 0) && Server.npcHandler.npcs[npcClickIndex] !=
		 * null) { if(goodDistance(getX(), getY(),
		 * Server.npcHandler.npcs[npcClickIndex].getX(),
		 * Server.npcHandler.npcs[npcClickIndex].getY(), 1)) { if(clickNpcType
		 * == 1) { turnPlayerTo(Server.npcHandler.npcs[npcClickIndex].getX(),
		 * Server.npcHandler.npcs[npcClickIndex].getY());
		 * Server.npcHandler.npcs[npcClickIndex].facePlayer(playerId);
		 * getActions().firstClickNpc(npcType); } if(clickNpcType == 2) {
		 * turnPlayerTo(Server.npcHandler.npcs[npcClickIndex].getX(),
		 * Server.npcHandler.npcs[npcClickIndex].getY());
		 * Server.npcHandler.npcs[npcClickIndex].facePlayer(playerId);
		 * getActions().secondClickNpc(npcType); } if(clickNpcType == 3) {
		 * turnPlayerTo(Server.npcHandler.npcs[npcClickIndex].getX(),
		 * Server.npcHandler.npcs[npcClickIndex].getY());
		 * Server.npcHandler.npcs[npcClickIndex].facePlayer(playerId);
		...
		/*
		 * if(walkingToItem) { if(getX() == pItemX && getY() == pItemY ||
		 * goodDistance(getX(), getY(), pItemX, pItemY,1)) { walkingToItem =
		 * false; Server.itemHandler.removeGroundItem(this, pItemId, pItemX,
		 * pItemY, true); } }
		 */
		 ...
		 * teleGrabItem, teleGrabX, teleGrabY, true); } }
		 */
		...
		if (attackTimer > 0)
		{
			if (playerEquipment[playerWeapon] != 20235)
				attackTimer--;
			else
				attackTimer -= 4;
			//Restart
	...
	public void handlePvpInterface() {
		if (inWild() && !isInPbox() && !isInArd() && !isInFala() && !inFunPk()) {
			...
			/*
			 * } else if (!inWild() && !inDuelArena() && !getPA().inPitsWait()
			 * && !inPits && safeTimer <= 0 && !inGWD() && !inPcBoat() &&
			 * !inPcGame()){ //this makes it so attack option is visible on wild
			 * and challenge in duel =) getPA().showOption(3, 0, "Throw at", 1);
			 * getPA().walkableInterface(-1);
			 */
			/*
			 * } else if (!inWild() && playerEquipment[playerWeapon] != 11951 &&
			 * !inDuelArena() && safeTimer <= 0 && !inGWD() && !inPcBoat() &&
			 * !inPcGame()){ //this makes it so attack option is visible on wild
			 * and challenge in duel =) getPA().showOption(3, 0, "View shop",
			 * 1); getPA().walkableInterface(-1);
			 */
			...
			getPA().walkableInterface(25133);
		} else if (!hasAchieved) {
			getPA().walkableInterface(-1);
		} else if (inFunPk()) {
			getPA().walkableInterface(197);
			getPA().sendFrame126("@red@PK Mini", 199);
			getPA().showOption(3, 0, "Attack", 1);
			wildLevel = 55;
		} else if (inPcGame()) {
			...
			/*
			 * } else if (InDung()){ getPA().sendFrame126("@gre@Dungeoneering",
			 * 199);
			 */
		} else if (inGWD()) {
			getPA().GWKC();
			...
			/*
			 * } else if(isInPbox()){ int modY = absY > 6400 ? absY - 6400 :
			 * absY; wildLevel = 12; getPA().walkableInterface(197);
			 * getPA().showOption(3, 0, "Attack", 1);
			 * if(Config.SINGLE_AND_MULTI_ZONES) { if(inMulti()) { HighAndLow();
			 * } else { HighAndLow(); } }
			 */
		} else if (isInArd()) {
			int modY = absY > 6400 ? absY - 6400 : absY;
	...
	/*
	 * public Woodcutting getWoodcutting() { return woodcutting; }
	 */
	...
	/*
	 * public Mining getMining() { return mine; }
	 */
	...
	public void correctCoordinates() {
		...
		/*
		 * if (inNomad()) { getPA().movePlayer(absX, absY, playerId * 4);
		 * sendMessage("Your wave will start in 10 seconds.");
		 * EventManager.getSingleton().addEvent(new Event() { public void
		 * execute(EventContainer c) {
		 * Server.fightCaves.spawnNextWave((Client)Server
		 * .playerHandler.players[playerId]); c.stop(); } }, 10000);
		 * 
		 * }
		 */
