...
public class PlayerSave {
			...
			if (spot > -1) {
				...
				case 2:
					if (token.equals("character-height")) {
						 ...
						 * Integer.parseInt(token2); } else if (token.equals("bhleave")) { p.enterBHTime
						 * = Integer.parseInt(token2); } else if (token.equals("bhpickup")) {
						 * p.enterBHTime = Integer.parseInt(token2); } else if (token.equals("bhcash"))
						 * { p.enterBHTime = Integer.parseInt(token2); } else if
						 * (token.equals("bhcrater")) { p.enterBHTime = Integer.parseInt(token2); } else
						 * if (token.equals("bhrogue")) { p.rogueKill = Integer.parseInt(token2); } else
						 * if (token.equals("bhtarget")) { p.targetKill = Integer.parseInt(token2);
						 */
						...
						/*
						 * } else if (token.equals("runeQuest")) { p.rMQ = Integer.parseInt(token2); }
						 * else if (token.equals("herbQuest")) { p.bMQ = Integer.parseInt(token2);
						 */
						...
						/*
						 * } else if (line.startsWith("KC")) { p.KC = Integer.parseInt(token2); } else
						 * if (line.startsWith("DC")) { p.DC = Integer.parseInt(token2);
						 */
						...
						/*
						 * } else if (token.equals("flagged")) { p.accountFlagged =
						 * Boolean.parseBoolean(token2); } else if (token.equals("Rules")) { p.readRules
						...
						/*
						 * } else if (token.equals("Black-marks")) { p.BlackMarks =
						 * Integer.parseInt(token2);
						 */
					}
					break;
	...
	public static boolean saveGame(Client p) {
		...
		if (p.playerName == null || PlayerHandler.getPlayers()[p.playerId] == null) {
		...
		try {
			...
			characterfile.write(Integer.toString(p.ForgottenWarriorKilled), 0,
					Integer.toString(p.ForgottenWarriorKilled).length());
			characterfile.newLine();
			...
			characterfile.write(Integer.toString(p.PredatorTorvaKilled), 0,
					Integer.toString(p.PredatorTorvaKilled).length());
			characterfile.newLine();
			 ...
			 * Boolean.toString(p.accountFlagged).length()); characterfile.newLine();
			...
			characterfile.write(Integer.toString(p.achievementPoints), 0,
					Integer.toString(p.achievementPoints).length());
			characterfile.newLine();
			...
			for (int i = 0; i < p.playerShop.length; i++) {
				if (p.playerShop[i] > 0) {
					...
					characterfile.write(Integer.toString(i), 0, Integer.toString(i).length());
					characterfile.write("	", 0, 1);
					characterfile.write(Integer.toString(p.playerShop[i]), 0,
							Integer.toString(p.playerShop[i]).length());
					characterfile.write("	", 0, 1);
					characterfile.write(Integer.toString(p.playerShopP[i]), 0,
							Integer.toString(p.playerShopP[i]).length());
					characterfile.write("	", 0, 1);
					characterfile.write(Integer.toString(p.playerShopN[i]), 0,
							Integer.toString(p.playerShopN[i]).length());
					characterfile.newLine();
				}
			}
			characterfile.newLine();
			...
			/* IGNORES */
			/*
			 * characterfile.write("[IGNORES]", 0, 9); characterfile.newLine(); for (int i =
			 * 0; i < ignores.length; i++) { if (ignores[i] > 0) {
