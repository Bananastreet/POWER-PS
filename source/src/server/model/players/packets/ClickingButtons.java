...
public class ClickingButtons implements PacketType {
	...
	public void processPacket(final Client c, int packetType, int packetSize) {
		...
		int[] spellIds = { 4128, 4130, 4132, 4134, 4136, 4139, 4142, 4145, 4148, 4151, 4153, 4157, 4159, 4161, 4164, 4165, 4129, 4133, 4137, 6006, 6007, 6026, 6036, 6046, 6056, 4147, 6003, 47005, 4166, 4167, 4168, 48157, 50193, 50187, 50101, 50061, 50163, 50211, 50119, 50081, 50151, 50199, 50111, 50071, 50175, 50223, 50129, 50091 };
		...
		case 161004:
			c.getPA().sendFrame126("www.power-ps.org", 12000);
			break;
		case 161005:
			c.getPA().sendFrame126("www.power-ps.org", 12000);
			break;
		case 161006:
			// c.getPA().sendFrame126("Voting currently disabled", 12000);
			c.sendMessage("Voting currently disabled.");
			break;
		case 161007:
			c.getPA().sendFrame126("http://rsps-pay.com/store.php?id=4190", 12000);
			c.sendMessage("Type ::claim after donating.");
			break;
		case 161022:
			c.getPA().sendFrame126("www.power-ps.org", 12000);
			break;
		...
		case 160242:
			...
			c.getPA().sendFrame126("Information", 41215);
			break;
		case 160245:
			c.setSidebarInterface(2, 41300);
			c.getPA().sendFrame126("PvP Statistics", 41215);
			c.getPA().sendFrame126("@or1@Kills: " + c.Killed + "", 41227);
			c.getPA().sendFrame126("@or1@Deaths: " + c.Death + "", 41228);
			double KDR = ((double) c.Killed) / ((double) c.Death);
			c.getPA().sendFrame126("@or1@Kill/Death Ratio: " + c.Killed + "/" + c.Death + "; " + KDR + "", 41229);
			c.getPA().sendFrame126("@or1@Pk Points: " + c.pkPoints + "", 41230);
			break;
		case 160248:
			c.setSidebarInterface(2, 41350);
			c.getPA().sendFrame126("Statistics", 41215);
			c.getPA().sendFrame126("@or1@Loyalty Points: " + c.LoyaltyPoints + " ", 41231); // @or1@Power-Ps
																							// Points:
																							// "+c.pcPoints+"
			c.getPA().sendFrame126("@or1@Dungeoneering Points: " + c.dungPoints + "", 41232);
			c.getPA().sendFrame126("@or1@Power-Ps Points: " + c.pcPoints + "", 41233);
			c.getPA().sendFrame126("@or1@Custom Points: " + c.customPoints + "", 41234);
			c.getPA().sendFrame126("@or1@Achievements: " + c.achievementPoints + "", 41235);
			// c.getPA().sendFrame126("@or1@Slayer points: "+c.slayerPoints+"",
			// 41235);//Achievements
			break;
		case 160251:
			c.setSidebarInterface(2, 41400);
			c.getPA().sendFrame126("Support", 41215);
			c.getPA().sendFrame126("@or1@Online Staff", 41236);
			c.getPA().sendFrame126("@or1@Call for help", 41237);
			c.getPA().sendFrame126("@or1@Help forum", 41238);
		...
		case 171227:
			...
			c.sendMessage("You have requested staff assistance.");
				...
				if (PlayerHandler.players[i] != null) {
					...
					c2.sendMessage("Alert##Notification##" + c.playerName + " is requesting staff assistance!" + "##Please do your best to assist " + c.playerName);
			}}}}
			break;
		case 171229:
			if (c.getItems().freeSlots() <= 1) {
		...
		case 176164: // Nomad&Angry goblin
			c.getPA().startTeleport(2763, 3486, 0, "modern");
			break;
		case 178037: // FUNPK
		...
		case 178040: // FUNPK
			...
			c.getPA().startTeleport(2090, 4428, 0, "modern"); // original
			break;
		...
		// BOSS TELEPORTS
		case 164023:
			c.getPA().startTeleport(3018, 5233, 0, "modern"); // HELL
			break;
		case 176167:
		case 178055:
		case 177211:
			...
			break;
		case 164029:
			c.getPA().startTeleport(2854, 3810, 0, "modern"); // torm demons
			break;
		case 164025:
			c.getPA().startTeleport(2463, 4774, 0, "modern"); // corp beast
			c.sendMessage("Warning: You may need a team.");
			break;
		case 164039:
			// AchievementManager.increase(c, Achievements.VENTURE);
			c.getPA().startTeleport(2507, 4714, 0, "modern"); // no npc in wild
			c.sendMessage("1:1200");			
		case 178043:
		...
		case 164027:
			c.getPA().startTeleport(2242, 3278, 0, "modern"); // K QUEEN
			break;
		case 175209:
			...
			break;
		case 175203:
			c.getPA().startTeleport(2794, 4722, 0, "modern"); // ventige wrecker
			break;

		case 175205:
			c.sendMessage("Coming soon!"); //aiyana
			break;
		case 175207:
			c.getPA().startTeleport(3283, 3429, 0, "modern"); // ventige wrecker
			break;
		case 175211:
			c.sendMessage("Coming soon!");
			break;
		...
		case 164019:
			c.getPA().startTeleport(3503, 3567, 0, "modern"); // hulk
			break;
		case 164033:
			c.getPA().startTeleport(3211, 3797, 0, "modern"); // barrelchest
			c.sendMessage("Wildy 35Skull Be careful!");
			break;
		case 164037:
			c.getPA().startTeleport(1928, 5002, 0, "modern"); // giant mole
			break;
		case 164035:
			c.getPA().startTeleport(2777, 9287, 0, "modern"); // asg
																// asg
																// asg
			break;
		case 164031:
			c.getPA().startTeleport(3306, 9198, 0, "modern"); // valgor
			break;
		case 164021:
			c.getDH().sendDialogues(5555, 1);
			break;
		...
		case 43209:
			if (c.LoyaltyPoints >= 1000) {
				...
				c.sendMessage("You Achieved 1000 Loyalty Points And Lootbox Has been Adeed to your inventory");				
			}
			break;
		case 59108:
		...
		case 137013:
			if (c.lampfix == 5) {
				...
				c.playerLevel[c.antiqueSelect] = 199;
				c.playerXP[c.antiqueSelect] = c.getPA().getXPForLevel(136);
				c.getPA().refreshSkill(c.antiqueSelect);
				// c.getPA().addSkillXP(1008438379,c.antiqueSelect);
		...
		case 58073:
			...
			c.getPA().sendFrame126("www.power-ps.org", -1);
		 ...
		 * 1); c.getItems().addItem(1067, 1); c.getItems().addItem(1115, 1);
		...
		case 82020:
			if(c.inTrade){
				c.sendMessage("cant bank items while in trade.");
				return;
			}
		 ...
		 * // 12435 c.getItems().addItem(15272, 4);
		...
		case 71074:
			 ...
			 * System.currentTimeMillis(); return; } } } else {
		...
		case 16117:
			...
			if (c.isDonator == 1 && !c.inWild() && !c.pkSafe()) {
				c.getPA().openUpBank();
				return;
			}
			break;
		...
		case 9157:
			...
			if (c.dialogueAction == 10010) {
				if (c.getItems().playerHasItem(1514, 150)) {
					c.getItems().deleteItem(1514, 150);
					c.getItems().addItem(15420, 1);
					AchievementManager.increase(c, Achievements.CWhip);
					c.sendMessage("You receive a Present.");
				} else {
			...
			if (c.dialogueAction == 10011) {
				if (c.getItems().playerHasItem(19468, 1) && c.getItems().playerHasItem(12272, 1) && c.getItems().playerHasItem(20472, 1) && c.getItems().playerHasItem(14036, 1) && c.getItems().playerHasItem(20471, 1)) {
					c.getItems().deleteItem(19468, 1);
					c.getItems().deleteItem(12272, 1);
					c.getItems().deleteItem(20472, 1);
					c.getItems().deleteItem(14036, 1);
					c.getItems().deleteItem(20471, 1);
					c.getItems().addItem(11308, 1);
					c.sendMessage("You receive a Perfect Sword.");
				} else {
					c.sendMessage("You don't have the required items.");
				}
			}
			...
			if (c.dialogueAction == 25002) {
				...
				// c.sendMessage("Under Construction - Coming very soon!");
			}
		...
		case 50056:
		case 117048:
			c.getPA().spellTeleport(2965, 3379, 0);
		...
		case 117112:
			...
			// "Slayer Tower", "Brimhaven Dungeon", "-More Options-");
			break;

		case 51013:
		case 6004:
		case 117162:
			c.getPA().startTeleport(3088, 3501, 0, "modern");
			break;
