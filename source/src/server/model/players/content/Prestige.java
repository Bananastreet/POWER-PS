...
public class Prestige {
	...
	public static boolean allowedToPrestige(Client c) {
		for (int i = 0; i < levelsToReset.length; i++) {
			if(c.getLevelForXP(c.playerXP[i]) < 110){
				c.sendMessage("You need 110 in all combat type skills before trying this.");
	...
	public static void initiatePrestige(Client c) {
		if(allowedToPrestige(c)) {
			...
			int rewardAmount = c.prestigeLevel*100000000;
			c.sendMessage("You are rewarded cash for resetting your stats.");
	...
	public static void openPrestigeShop(Client c) {
		if(allowedToShop(c)) {
			c.getShops().openShop(103+getCurrentPrestige(c));
