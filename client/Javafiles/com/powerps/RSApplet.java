...
import com.powerps.Client.ScreenMode;
...
public class RSApplet extends Applet implements Runnable, MouseListener, MouseMotionListener, KeyListener,
	...
	public void mouseWheelMoved(MouseWheelEvent event) {
		...
		if (mouseX > 0 && mouseX < 512 && mouseY > 342 + Client.instance.getChatBackIncreaseY()) {
	...
	/*
	 * public final void mouseDragged(MouseEvent mouseevent) { int i =
	 * mouseevent.getX(); int j = mouseevent.getY(); if(gameFrame != null) { Insets
	 * insets = gameFrame.getInsets(); i -= insets.left; j -= insets.top; } idleTime
	 * = 0; mouseX = i; mouseY = j; }
	 */
			...
			if (KeyEvent.getKeyModifiersText(keyevent.getModifiers()).contains("Ctrl")) {
				...
				if (Client.newDamage)
					JOptionPane.showMessageDialog(null,
							"You are now using the new damage system (Hits are multiplied by 10).", "Tyrena Client", 1);
				else
					JOptionPane.showMessageDialog(null,
							"You are now using the old damage system (Hits are not multiplied by 10).", "Tyrena Client",
							1);
			}
		}
		if (hotKey == 508) {
	...
	public void setCursor(int id) {
		Image image = getGameComponent().getToolkit().createImage(
				FileOperations.ReadFile(signlink.findcachedir() + "Sprites/Hack3r/Cursors/Cursor " + id + ".PNG"));
