...
public final class ItemDef {
	...
	public int almighty = 0;
	public int almighty2 = 0;
	public int almighty3 = 0;
	...
	public static void itemModel() {
		try {
			for (int i = 0; i < 20793; i++) {
				...
				byte abyte0121[] = FileOperations.ReadFile(
						(new StringBuilder()).append("./items/").append(itemDef.modelID).append(".mdl").toString());
				FileOperations.WriteFile(
						(new StringBuilder()).append("./dumped/").append(itemDef.modelID).append(".mdl").toString(),
						abyte0121);
			}
	...
	private void toLend() {
		...
		modelSymbols = itemDef_1.modelSymbols;
		femaleEquip1 = itemDef_1.femaleEquip1;
		modelSize = itemDef_1.modelSize;
	...
	public Model method194(int j) {
		...
		if (almighty > 0)
			model.method1337(almighty);
		if (almighty2 != 0)
			model.method1338(almighty2);
		if (almighty3 != 0)
			model.method1339(almighty3);
		return model;
	}
	...
	public Model method196(int i) {
		...
		if (i == 0 && modelHeight1 != 0)
			model.method475(0, modelHeight1, 0);
		if (i == 1 && modelHeight2 != 0)
			model.method475(0, modelHeight2, 0);
		if (modifiedModelColors != null) {
			for (int i1 = 0; i1 < modifiedModelColors.length; i1++)
				model.method476(modifiedModelColors[i1], originalModelColors[i1]);
	...
	private void readValues(Stream stream) {
		do {
			...
			if (i == 1)
				...
				modelHeight1 = stream.readSignedByte();
			} else if (i == 24)
				anInt188 = stream.readUnsignedWord();
			else if (i == 25) {
				femaleEquip1 = stream.readUnsignedWord();
				modelHeight2 = stream.readSignedByte();
			} else if (i == 26)
				...
				modelSymbols = stream.readUnsignedWord();
			else if (i == 79)
				...
				modelSize = stream.readUnsignedWord();
			else if (i == 95)
				...
				modelBrightness = stream.readSignedByte();
			else if (i == 114)
				modelShadow = stream.readSignedByte() * 5;
			else if (i == 115)
	...
	public static ItemDef forID(int i) {
		...
		itemDef = ItemDef_2.itemDef(i, itemDef);// ItemDef_3
		itemDef = ItemDef_3.itemDef(i, itemDef);// ItemDef_3
		itemDef = ItemDef_4.itemDef(i, itemDef);// ItemDef_4
		if (itemDef.certTemplateID != -1)
			itemDef.toNote();
	...
	public void toNote() {
		...
		if (c == 'A' || c == 'E' || c == 'I' || c == 'O' || c == 'U')
			s = "an";
		description = (new StringBuilder()).append("Swap this note at any bank for ").append(s).append(" ")
				.append(itemDef_1.name).append(".").toString();
		stackable = true;
	}
	...
	public Model getModel(int i) {
		...
		model.method479(64 + modelBrightness, 768 + modelShadow, -50, -10, -50, true);
