...
final class CollisionMap {
	...
	public void removeFloorDecoration(int y, int x) {
		x -= xOffset;
		y -= yOffset;
	...
	public boolean method221(int i, int j, int k, int l, int i1, int j1, int k1) {
		...
		if (k >= j && k <= l1 && k1 >= i && k1 <= i2)
			return true;
		if (k == j - 1 && k1 >= i && k1 <= i2 && (adjacencies[k - xOffset][k1 - yOffset] & 8) == 0
				&& (i1 & 8) == 0)
			return true;
		if (k == l1 + 1 && k1 >= i && k1 <= i2 && (adjacencies[k - xOffset][k1 - yOffset] & 0x80) == 0
				&& (i1 & 2) == 0)
			return true;
		else
			return k1 == i - 1 && k >= j && k <= l1 && (adjacencies[k - xOffset][k1 - yOffset] & 2) == 0
					&& (i1 & 4) == 0
					|| k1 == i2 + 1 && k >= j && k <= l1
							&& (adjacencies[k - xOffset][k1 - yOffset] & 0x20) == 0 && (i1 & 1) == 0;
	}
}
